...
# file_path = "../KG Construction/RE-TACRED/prompts/retacred-0-shot-prompt.txt"
file_path = "../KG Construction/RE-TACRED/prompts/retacred-0-shot-prompt-opt.txt"
# file_path = "../KG Construction/RE-TACRED/prompts/retacred-1-shot-prompt.txt"
# file_path = "../KG Construction/RE-TACRED/prompts/retacred-1-shot-prompt-opt.txt"
...
for prompt in prompts:
    question_match = re.search(r"The list of predicates: (.+?)RE-TACRED:", prompt, re.DOTALL)
    if question_match:
        question = question_match.group(1).strip()  # 去除前后空格
        questions.append(question)

    answer_match = re.search(r"RE-TACRED\s*:\s*(.*)", prompt, re.DOTALL)
    if answer_match:
        answer_text = answer_match.group(1).strip()
        answer_spo_match = re.search(r"\[(.*?)\]", answer_text)
        if answer_spo_match:
            answer_spo = f"[{answer_spo_match.group(1)}]"
            answer_spo = answer_spo.replace('\\', '\\\\')
            answer_spo_list = [ast.literal_eval(fix_spo(answer_spo))]
...
questions_temp = questions[:5]
answers_temp = answers[:5]
...
def calculate_f1(y_true, y_pred):
    ...
    print("y_true_set: ", y_true_set)
    print("y_pred_set: ", y_pred_set)
